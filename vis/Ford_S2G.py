...
class Ford_S2G(nn.Module):
    def __init__(self, args):  # device='cuda:0',
        super(Ford_S2G, self).__init__()
        '''
    ...
    def grd_f2BEV(self, image, optical):
        ...
        assert torch.sum(mask) > 0#存在数合适

        nw = (ix_se - ix) * (iy_se - iy) * mask#改变量
        ...
        nw_val = torch.gather(image, 2, (iy_nw * IW + ix_nw).long().view(N, 1, H * W).repeat(1, C, 1)).view(N, C, H, W)#改变后的图像
    ...
    #N点法 Least-Squares Fitting of Two 3-D Point Sets
    ...
    def forward(self, sat_map, grd_img_left, R_FL, T_FL):
        '''
        Args:
        ...
        sat_feat = sat_map
        di = ori_grdW/512
        grd_feat = F.interpolate(grd_img_left, size=(int(ori_grdH/di), 512), mode='bilinear', align_corners=False)
