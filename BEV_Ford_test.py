...
from vis.Ford_S2G import Ford_S2G
...
def test1(net_test, args, save_path, best_rank_result, epoch):
    ...
    test_set = SatGrdDatasetFordTest(logs=test_logs[args.test_log_ind:args.test_log_ind+1],
                                     logs_img_inds=test_logs_img_inds[args.test_log_ind:args.test_log_ind+1],
                                     shift_range_lat=args.shift_range_lat,
                                     shift_range_lon=args.shift_range_lon,
                                     rotation_range=args.rotation_range, whole=0)
    dataloader = DataLoader(test_set, batch_size=mini_batch, shuffle=False, pin_memory=True,
    ...
    for i, data in enumerate(dataloader, 0):
        # if i!=2 and i!=29 and i!=65 and i!=82 and i!=87 and i!=88 and i!=99:
        #     continue
        ...
        if args.end2end == 0:
            flow_predictions, flow_conf, mask = net(int(epoch), sat_map, grd_img, R_FL, T_FL, gt_shift_u, gt_shift_v, gt_heading, mode='train')
            mask = mask.float()
        ...
        if args.end2end == 1:
            flow_predictions, flow_conf, mask, pre_u, pre_v, pre_theta = net(int(epoch), sat_map, grd_img, R_FL, T_FL, gt_shift_u, gt_shift_v, gt_heading, mode='train', file_name=0)
            ...
            #         for w in range(coords1_gt.size()[1]):
            #             if ((h == 270 and w == 340) or \
            #                 (random.randint(0,2000) == 1)) and mask[0,0,h,w]:
            #                 x = [coords0_gt[h][w][0].data.float().cpu(), coords1_gt[h][w][0].data.float().cpu() + coords1_gt.size()[1]]
            #                 y = [coords0_gt[h][w][1].data.float().cpu(), coords1_gt[h][w][1].data.float().cpu()]
            #                 match_x.append(x)
            #                 match_y.append(y)
            #     line_point('result_visualize/left_half.jpg', 'result_visualize/right_half.jpg', match_x, match_y,None, 'line_pre.jpg')
        ...
        if abs(gt_shift - shifts).sum()<5:
            print(i)
        # s2g = Ford_S2G(args)
        # uv = s2g(sat_map, grd_img, R_FL, T_FL)
        ...
        # RGB_Ford_com_pose(sat_map, grd_img, gt_u=vis_u, gt_v=vis_v, gt_theta = vis_heading,\
        #     pre_u = shifts[:,1]/ 0.22, pre_v =  shifts[:,0]/ 0.22,pre_theta=pre_theta[:,0],\
        #     com_u = mcc_v/ 0.22, com_v = -mcc_u/ 0.22,come_theta=-mcc_thate,\
        #     save_dir='./result_visualize/')
        # RGB_Ford_pose(sat_map, grd_img, gt_u=vis_u, gt_v=vis_v, gt_theta = vis_heading,\
        #     pre_u = shifts[:,1]/ 0.22, pre_v =  shifts[:,0]/ 0.22,pre_theta=pre_theta[:,0],\
        #     com_u = mcc_v/ 0.22, com_v = -mcc_u/ 0.22,come_theta=-mcc_thate,\
        #     save_dir='./result_visualize/')
...
def parse_args():
    ...
    parser.add_argument('--train_log_end', type=int, default=3, help='')
